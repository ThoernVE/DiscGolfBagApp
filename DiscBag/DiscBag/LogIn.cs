...
namespace DiscBag
{
    internal class LogIn
    {
        ...
        private static void LogInMethod() //method that takes two userinputs and checks them against the list to make sure both password and usernames are the same as input. Not functioning, needs rework?
        {
            ...
            if (existingUser == true)
            {
                for (int i = 0; i < UserClass.userNames.Count; i++) //checks every iteration of username and password to make sure both sides match
                {
                    ...
                    else if (attemptedUsername == UserClass.userNames[i] && attemptedPassword != UserClass.passwords[i])
                    {
        ...
        {
            var serializedObject = Newtonsoft.Json.JsonConvert.SerializeObject(UserClass.userNames);
        ...
        {
            var serializedObject = Newtonsoft.Json.JsonConvert.SerializeObject(UserClass.passwords);
            string path = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
        ...
        public static void LoadUserNames()
        {
            try
            {
                ...
                List<string> loadedUsers = JsonConvert.DeserializeObject<List<string>>(content);

                foreach (string user in loadedUsers)
                {
        ...
        public static void LoadPasswords()
        {
            try
            {
                ...
                List<string> loadedPasswords = JsonConvert.DeserializeObject<List<string>>(content);

                foreach (string password in loadedPasswords)
                {
